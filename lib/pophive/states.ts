// Standard US Census state FIPS codes, names, and postal abbreviations.
// Used to cross-walk between PopHIVE's full-state-name rows (overall_trends
// files) and us-atlas's 2-digit zero-padded FIPS ids (TopoJSON `id`).
export const US_STATES: { fips: string; name: string; abbr: string }[] = [
...
export const STATE_FIPS_BY_NAME: Record<string, string> = Object.fromEntries(
  US_STATES.map((s) => [s.name, s.fips])
);

export const STATE_NAME_BY_FIPS: Record<string, string> = Object.fromEntries(
  US_STATES.map((s) => [s.fips, s.name])
);
